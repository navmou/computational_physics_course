...
{
  /*Files to save the trajectories to calculate the correlation functions*/
  ...
  double s1 , s2 , r; /*dummy vaiables to do the summations*/
  ...
  */
  ...
  for (int l = 0 ; l < 50 ; ++l) {
    ...
    for (int p = 0 ; p < n_particles; ++p) {
      c1 = 0.0;
      for (int m = 0; m < M - l ; ++m) {
	c1 += (R[m+l+10000][p] - R[m+10000][p])*(R[m+l+10000][p] - R[m+10000][p]);
	printf("%f\n",c1 );
      }
      c2 += c1/(double) (M-l);
    }
