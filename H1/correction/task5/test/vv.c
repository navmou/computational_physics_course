...
and all other calculatins in the simulation.
Written by NavidMousavi
*/
		     ...
		     double T_eq , double P_eq , double tau , double check_particles[][5] , double *L_list)
{
  ...
  FILE *pos_tot = fopen("pos-total.txt" , "w");
  FILE *pos_r = fopen("r.txt", "w");
  double (*R)[n_particles] = malloc(sizeof(double [n_timesteps][n_particles]));
  double (*V)[256][3] = malloc(sizeof(double [n_timesteps][n_particles][3]));
  printf("No problem\n");
  double s1 , s2 , r;
  ...
  L_list[0] = L;
  /*
    Initializing the check_particles with the initial positions of selected atoms
    atoms 0,50,100,150,200
  */
  
  for (int i = 0; i < n_particles; ++i) {
    r = 0.0;
    for (int j = 0 ; j < 3; ++j) {
      fprintf(pos_tot, "%f\t", pos[i][j]);
      r+=pos[i][j]*pos[i][j];
    }
    fprintf(pos_r, "%f\t", sqrt(r));
    R[0][i] = sqrt(r);
  }
  for (int i = 0; i < n_particles; ++i) {
    for (int j = 0 ; j < 3; ++j) {
  ...
  check_particles[0][0] = pos[0][0];
  check_particles[0][1] = pos[50][0];
  check_particles[0][2] = pos[100][0];
  check_particles[0][3] = pos[150][0];
  check_particles[0][4] = pos[200][0];
  ...
  for (int t = 1; t < n_timesteps + 1; ++t)
	...
	for (int j = 0 ; j < 3; ++j) {
      ...
      fprintf(pos_tot, "\n");
      fprintf(pos_r, "\n");
      /*updating the check_particles*/
      check_particles[t][0] = pos[0][0];
      check_particles[t][1] = pos[50][0];
      check_particles[t][2] = pos[100][0];
      check_particles[t][3] = pos[150][0];
      check_particles[t][4] = pos[200][0];
    }
  
  //MSD calculation
  printf("Calculating the MSD: \n");
  double c1 , c2 ;
  FILE *cl = fopen("c_l.txt" , "w");
  int M = n_timesteps;
  for (int l = 0 ; l < 5000 ; ++l) {
    c2 = 0.0;
    for (int p = 0 ; p < n_particles; ++p) {
      c1 = 0.0;
      for (int m = 10000; m < M-l ; ++m) {
	c1 += (R[m+l][p] - R[m][p])*(R[m+l][p] - R[m][p]);
	//printf("%f\n",c1 );
      }
      c2 += c1/(double) (M-l-10000);
    }
    fprintf(cl, "%f\n", c2/(double) n_particles);
  }
  fclose(cl);

  //Velocity correlation calculation
  printf("Calculating Velocity correlaiton: \n");
  FILE *vl = fopen("v_l.txt" , "w");
  M = n_timesteps;
  for (int l = 0 ; l < 5000 ; ++l) {
    c2 = 0.0;
    for (int p = 0 ; p < n_particles; ++p) {
      c1 = 0.0;
      for (int m = 10000; m < M-l ; ++m) {
	c1 += (V[m+l][p][0]*V[m][p][0] + V[m+l][p][1]*V[m][p][1] + V[m+l][p][2]*V[m][p][2]);
	//printf("%f\n",c1 );
      }
      c2 += c1/(double) (M-l-10000);
    }
