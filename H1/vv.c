...
void velocity_verlet(int n_timesteps, int n_particles, double v[][3], double pos[][3],
		     ...
		     double T_eq , double P_eq , double tau , double check_particles[][5])
{
  ...
  */
  ...
  for (int i = 0; i < n_particles; ++i) {
    fprintf(pos_tot, "x%d\ty%d\tz%d\t", i , i , i);
...
void write_to_file(char *fname,  double *kinE , double *potE ,
		   ...
		   double *Temperature , int n_timesteps)
{
  FILE *fp = fopen(fname, "w");
  for(int i = 0; i < n_timesteps; ++i){
    fprintf(fp, "%f\t%f\t%f\t%f\t%f\n", kinE[i] , potE[i] , totE[i], Pressure[i] , Temperature[i]);
  }
  fclose(fp);
}
